...
class CTStack(Stack):
    ...
    #         if np.any(include):
    #             # include_2d = include[:,:,0]
    #             # x, y = np.where(include_2d)
    ...
    def import_stack(self, low=0, high=None):
        ...
        low : int, default=0
            The minimum value for an inclusive filter.
        high : int, default=None
            The maximum value for an inclusive filter, defaulting to 
            the maximum.
        """
        for lbl in ['points', 'points_original', 'points_subset']:
            if lbl in dir(self):
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            layers = list(executor.map(load_layer, self.iter_layers()))
        ...
        for num, layer in enumerate(layers):
            ...
            if np.any(include):
                ...
                pts = np.array([
                    np.repeat(float(num) * self.depth_size, len(x)),
                    self.pixel_size * x.astype(float),
                    self.pixel_size * y.astype(float)
                ]).T
                for dataset in [self.points, self.points_original]:
                    dataset.resize((dataset.shape[0] + len(x), 3))
                    dataset[-len(x):] = pts
                num_points = len(x)
                sample = math.ceil(0.01 * num_points)
                inds = np.random.choice(np.arange(num_points), size=sample, replace=False)
                self.points_subset.resize((self.points_subset.shape[0] + sample, 3))
                self.points_subset[-sample:] = pts[inds]
            print_progress(num, len(self.fns))
        print()
