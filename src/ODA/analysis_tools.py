...
class CTStack(Stack):
                ...
                if np.sum(in_window) > 30:
                    ...
                    if sum(in_shell) > 10:
                        ...
                        for lbl in lbls_set:
                            ...
                            print("Subset original size:", subset_original.shape)
                            print("Indices size:", inds.shape)
                            # 调试错误发生前的数组操作
                            if subset_original.shape[0] != inds.shape[0]:
                                raise ValueError("Size mismatch: subset_original size is {} but inds size is {}".format(subset_original.shape, inds.shape))
    ...
    def ommatidia_detecting_algorithm(self, polar_clustering=True, display=False,
        ...
        if stage < 1:
            ...
            if not prefiltered:
                self.gui = StackFilter(self.fns)
                print('Select the limits for the stack......')
                low, high = self.gui.get_limits()
            else:
